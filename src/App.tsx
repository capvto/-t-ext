...
useEffect(() => {
  ...
  const getPublicIdFromPathname = (pathname: string) => {
    ...
    let seg = "";
    try {
      seg = decodeURIComponent(mRoot[1] || "");
    } catch {
      seg = mRoot[1] || "";
    }

    // Keep the editor at `/` and avoid treating well-known static paths as public pages.
    const reserved = new Set(["p", "assets", "api", "favicon.ico", "robots.txt", "sitemap.xml", "index.html"]);
    if (!seg || seg === "") return "";
  ...
  const [publicId, setPublicId] = useState<string>(() => {
    try {
      return getPublicIdFromPathname(window.location.pathname);
    } catch {
      return "";
    }
  });
  ...
  const navigate = (path: string) => {
    ...
    setPublicId(getPublicIdFromPathname(path));
  };

  useEffect(() => {
    const onPop = () => {
      try {
        setPublicId(getPublicIdFromPathname(window.location.pathname));
      } catch {
        setPublicId("");
      }
    };
