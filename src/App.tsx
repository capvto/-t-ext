...
type GlobalSettings = { css: string; themeId: string; fontId: string; monoFontId: string };

function loadGlobalSettings(): GlobalSettings {
  try {
    const raw = localStorage.getItem(GLOBAL_SETTINGS_KEY);
    if (!raw) return { css: "", themeId: "default", fontId: "default", monoFontId: "default" };
    ...
    return { css: "", themeId: "default", fontId: "default", monoFontId: "default" };
  }
}
...
// Persist global personalization (CSS / theme / fonts) separately.
useEffect(() => {
  const payload = JSON.stringify({ css: appCss, themeId: appThemeId, fontId: appFontId, monoFontId: appMonoFontId });
  safeLsSet(GLOBAL_SETTINGS_KEY, payload);
  fireAndForget(window.electronAPI?.kvSet?.(GLOBAL_SETTINGS_KEY, payload));
}, [appCss, appThemeId, appFontId, appMonoFontId]);
  ...
  return (
    ...
    >
      ...
      <>
        ...
        <div
          aria-hidden="true"
          className="hover-hotspot absolute left-0 top-0 z-30 h-full w-3"
          onMouseEnter={() => {
      ...
      >
        ...
        <PublicPasteClassicEditorPage
          ...
          onUpdateAppSettings={({ css, themeId, fontId, monoFontId }) => {
            setAppCss(css ?? "");
