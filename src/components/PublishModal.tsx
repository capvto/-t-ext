...
export default function PublishModal({ title, content, onClose }: Props) {
  ...
  const [result, setResult] = useState<{ viewUrl: string; editUrl: string; editCode: string } | null>(null);
  ...
  const exampleOrigin = origin || "text.matteocaputo.dev";
  ...
  const editUrl = result?.editUrl?.startsWith("/") && origin ? origin + result.editUrl : result?.editUrl || "";
          ...
          {!result ? (
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      setResult({ viewUrl: r.viewUrl, editUrl: r.editUrl, editCode: r.editCode });
                    } catch (e: any) {
            ...
            <>
              <div className="space-y-3">
                ...
                />
                <Field
                  label={t("publish.editLink")}
                  value={editUrl}
                  copied={copied === "edit"}
                  onCopy={async () => {
                    const ok = await copyToClipboard(editUrl);
                    if (ok) {
                      setCopied("edit");
                      setTimeout(() => setCopied(""), 1200);
                    }
