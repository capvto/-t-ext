...
type Props = {
  ...
  monoFontId: string;
  onClose: () => void;
  onApply: (patch: { css: string; themeId: string; fontId: string; monoFontId: string }) => void;
};
...
export default function AppPersonalizeModal({
  ...
  }, [initialCss, initialThemeId, initialFontId, initialMonoFontId]);
  ...
  function commit(next?: Partial<{ css: string; themeId: string; fontId: string; monoFontId: string }>) {
    const patch = {
      css: next?.css ?? css,
  ...
  function clearAll() {
    ...
    commit({ css: "", themeId: "default", fontId: "default", monoFontId: "default" });
  }
