...
### Removed
- **Modalità a blocchi rimossa**: l'editor è ora sempre un unico documento continuo. Il meccanismo click-to-activate per blocco, la divisione del testo in blocchi separati e il toggle nelle impostazioni sono stati eliminati.

### Added
- **Supporto mobile**: altezza viewport dinamica (`100dvh`) per gestire le barre dinamiche del browser; `viewport-fit=cover` + `safe-area-inset` per il notch su iPhone; scroll automatico della textarea all'apertura della tastiera virtuale; touch target minimi di 44 px su tutti i controlli.
- **Topbar mobile**: i bottoni secondari (Nuovo, Export, Import, Pubblica) si nascondono su schermi piccoli per tenere la topbar pulita, lasciando visibili solo i controlli essenziali.
- **Performance bundle**: `highlight.js` carica ora solo i linguaggi effettivamente utilizzati (~20) invece dell'intera libreria `common` (~40 linguaggi). Risparmio stimato: 60–80 KB nel bundle produzione.

### Changed
- Touch UX: rimossa la latenza dei tap (300 ms) su tutti i controlli interattivi; scrollbar nascosta su dispositivi touch; hotspot hover della sidebar disabilitato su touch screen.
